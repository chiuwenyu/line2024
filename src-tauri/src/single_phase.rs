...
impl Serialize for SingleFx {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let mut state = serializer.serialize_struct("SingleFx", 11)?;
        ...
        state.serialize_field("nre", &self.nre)?;
        state.serialize_field("fdarcy", &self.fdarcy)?;
        state.serialize_field("dp100", &self.dp100)?;
        state.serialize_field("vh", &self.vh)?;
        state.end()
    }
}
...
impl SingleFx {
    ...
    fn pressure_drop_100(&mut self) -> Result<f64, &'static str> {
        ...
        if self.rho * self.id * self.mu != 0.0 {
            ...
            self.dp100 = self.fdarcy * self.rho * self.v * self.v / (2.0 * g * self.id) / 10000.0
                * 100.0
                * self.sf;
            ret = Ok(self.dp100);
        } else {
    ...
    pub fn hydraulic(&mut self) {
        ...
        self.darcy_friction_factor()
            .expect("Darcy Friction Factor calculation error");
        self.pressure_drop_100()
            .expect("Pressure Drop calculation error");
        self.velocity_head()
            .expect("Velocity Head calculation error");
    }
}
